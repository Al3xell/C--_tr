...
void Window::add(std::unique_ptr<Shape> shape){shapes_.emplace_back(std::move(shape));}

void Window::drawAll_() 
  {
    for(auto& elem : shapes_) 
    {
      Shape *shape = dynamic_cast<Shape*>(elem.get());
      if(shape) shape->draw(win_);
    }
  }
void Window::moveAll_(double width,double height, double dt)
  {
    ...
    {
      Shape *shape = dynamic_cast<Shape*>(elem.get());
      if(shape) shape->move(width,height,dt);
    }
  }
